...
def load_frame(framecount, file_name, directory):
	frame = cv2.imread(directory+'/'+file_name, -1)
	framegray = frame.copy()
	framegray *= (1/256.) #resim0.convertTo(resim0,CV_8U,1/256.)
...
# 	avgheight.append(GPS_height)
	...
	resim0[mask] = 0 
	return frame, framegray, resim0 #, thresh
    
########### Find contours and calculate parameters #########	
def cont(resim0, framegray, frame, framecount):
	intlist_frame = []
	arealist_frame = []
	print intlist
	...
	for i in range(len(contours)):
		...
		print intlist_frame
		arealist_frame.append(len(pts[0]))
	for i in range(len(contours)): 
	...
	plot_cont(frame, framecount, frame.max(), frame.min()))
	return resim, contours, intlist_frame, arealist_frame

########## Display frame with contours ###########
def plot_cont(frame, framecount):
	plt.figure('Track')
 	plt.imshow(frame, vmin=frame.min(),vmax=frame.max())
...
for ifile,file in enumerate(filelist):
	frame, framegray, resim0 = load_frame(ifile, file, directory)
	framelist.append(frame)
	#maxframe, minframe = limits(frame)
	resim, contours, intlist_frame, arealist_frame = cont(resim0, framegray, frame, ifile)
